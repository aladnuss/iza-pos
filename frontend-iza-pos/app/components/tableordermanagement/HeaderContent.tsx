  ...
  return (
    <div className="flex items-center justify-between w-full bg-[var(--color-black)] border border-[var(--color-card-border)] rounded-3xl px-4 mt-2 py-2 shadow-sm">
      {!isExpanded ? (
        ...
        // Expanded mode - show only search bar and notif
        <>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onMinimize}
              className="p-2 rounded-xl bg-[var(--color-dark)] border border-[var(--color-card-border)] hover:bg-[var(--color-gray)] transition-colors"
              title="Minimize"
            >
              <Minimize className="w-5 h-5 text-[var(--color-foreground)]" />
            </button>
          </div>
        </>
      )}
